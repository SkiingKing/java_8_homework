...
class OrderStats {
    ...
    static List<Order> ordersForCardType(final Stream<Customer> customers, PaymentInfo.CardType cardType) {
        List<Order> list = customers
                            .flatMap(s ->  s.getOrders().stream())
                            .collect(Collectors.toList());
        return list.stream().filter(a -> a.getPaymentInfo().getCardType().equals(cardType)).collect(Collectors.toList());
    }
    ...
    static Map<Integer, List<Order>> orderSizes(final Stream<Order> orders) {
         return orders.collect(
                    Collectors.groupingBy(o->o.getOrderItems().stream().mapToInt(OrderItem::getQuantity).sum()));
    }
    ...
    static Boolean hasColorProduct(final Stream<Order> orders, final Product.Color color) {
        return orders.map(x -> x.getOrderItems()).allMatch(x -> x.stream().anyMatch(el -> el.getProduct().getColor() == color) == true);
    }
 ...
 //       return customers.collect(Collectors.groupingBy(Customer::getEmail,Collectors.flatMapping(o));
//       return orders.collect(
//                    Collectors.groupingBy(o->o.getOrderItems().stream().mapToInt(OrderItem::getQuantity).sum()));
        return null;
    }
